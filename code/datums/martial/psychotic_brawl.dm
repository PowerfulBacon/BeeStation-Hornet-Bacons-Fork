...
/datum/martial_art/psychotic_brawling/proc/psycho_attack(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	switch(rand(1,8))
		...
		if(4)
			...
			D.apply_damage(rand(5,10), BODY_ZONE_HEAD, BLUNT, A.body.get_bodypart(SKULL))
			A.apply_damage(rand(5,10), BODY_ZONE_HEAD, BLUNT, A.body.get_bodypart(SKULL))
			//TODO
		...
		if(5,6)
			...
			if(arm)
				D.apply_damage(rand(15,30), BODY_ZONE_CHEST, arm.punch_damage_type, arm)
