...
/datum/component/butchering/proc/onItemAttack(obj/item/source, mob/living/M, mob/living/user)
	...
	if(user.a_intent == INTENT_GRAB && ishuman(M) && source.is_sharp())
		...
		if(H.has_status_effect(/datum/status_effect/neck_slice))
			...
			return COMPONENT_ITEM_NO_ATTACK
		if((H.stat >= SOFT_CRIT || (user.pulling == H && user.grab_state >= GRAB_NECK) || H.IsSleeping()) && user.zone_selected == BODY_ZONE_HEAD) // Only sleeping, neck grabbed, or crit, can be sliced.
...
/datum/component/butchering/proc/startNeckSlice(obj/item/source, mob/living/carbon/human/H, mob/living/user)
	...
	if(do_mob(user, H, CLAMP(500 / source.force, 30, 100)) && H.Adjacent(source))
		...
		H.apply_damage_to(source.force, NECK, SLIT, source)
