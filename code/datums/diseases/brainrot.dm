...
/datum/disease/brainrot/stage_act() //Removed toxloss because damaging diseases are pretty horrible. Last round it killed the entire station because the cure didn't work -- Urist -ACTUALLY Removed rather than commented out, I don't see it returning - RR
	...
	switch(stage)
		if(2)
			...
			if(prob(5))
				affected_mob.body.apply_injury(BP_BRAIN, /datum/injury/rot, 0.1, 5)
		...
		if(3)
			...
			if(prob(10))
				affected_mob.body.apply_injury(BP_BRAIN, /datum/injury/rot, 0.2, 10)
				affected_mob.updatehealth()
				if(prob(2))
		...
		if(4)
			...
			if(prob(2))
				affected_mob.emote("drool")
			if(prob(15))
				affected_mob.body.apply_injury(BP_BRAIN, /datum/injury/rot, 0.3, 13)
				affected_mob.updatehealth()
				if(prob(2))
