...
/datum/map_template/proc/load(turf/T, centered = FALSE, init_atmos = TRUE, finalize = TRUE, ...)
	...
	map_placer.on_completion(CALLBACK(src, PROC_REF(on_placement_completed), T, init_atmos, parsed, finalize))
	map_placer.generate()
	return map_placer

/datum/map_template/proc/on_placement_completed(turf/T, init_atmos, datum/parsed_map/parsed, finalize = TRUE)
	var/list/bounds = parsed.bounds
	if(!bounds)
		maps_loading --
		if (!maps_loading)
