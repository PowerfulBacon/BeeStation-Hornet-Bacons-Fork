		...
		if(!istype(loc, /obj/machinery/atmospherics/components/unary/cryo_cell))
			...
			switch(bodytemperature)
				if(200 to BODYTEMP_COLD_DAMAGE_LIMIT)
					...
					add_overall_injury(/datum/injury/burn/freeze, COLD_DAMAGE_LEVEL_1, INJURY_SEVERITY_MINOR, 0)
				if(120 to 200)
					throw_alert("temp", /atom/movable/screen/alert/cold, 2)
					add_overall_injury(/datum/injury/burn/freeze, COLD_DAMAGE_LEVEL_2, INJURY_SEVERITY_MINOR, 0)
				if(-INFINITY to 120)
					throw_alert("temp", /atom/movable/screen/alert/cold, 3)
					add_overall_injury(/datum/injury/burn/freeze, COLD_DAMAGE_LEVEL_3, INJURY_SEVERITY_MINOR, 0)
		else
			clear_alert("temp")
