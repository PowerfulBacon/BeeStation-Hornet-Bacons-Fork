...
/mob/living/carbon/monkey/proc/handle_combat()
	...
	switch(mode)
		...
		if(MONKEY_HUNT)		// hunting for attacker
			...
			// switch targets
			for(var/mob/living/L in around)
				if(L != target && should_target(L) && L.body.stat == CONSCIOUS && prob(MONKEY_SWITCH_TARGET_PROB))
					target = L
					return TRUE
			...
			if(target && target.body.stat == CONSCIOUS)		// make sure target exists
		...
		if(MONKEY_FLEE)
			...
			for(var/mob/living/L in view(MONKEY_FLEE_VISION, src))
				if( enemies[L] && L.body.stat == CONSCIOUS )
					target = L
