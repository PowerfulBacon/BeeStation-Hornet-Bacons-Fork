...
/mob/living/proc/IsStun() //If we're stunned
	return body.has_status_effect(STATUS_EFFECT_STUN)
...
/mob/living/proc/Stun(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if(((status_flags & CANSTUN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		...
		if(S)
			...
			S = body.apply_status_effect(STATUS_EFFECT_STUN, amount, updating)
		return S
...
/mob/living/proc/IsKnockdown() //If we're knocked down
	return body.has_status_effect(STATUS_EFFECT_KNOCKDOWN)
...
/mob/living/proc/Knockdown(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		...
		if(K)
			...
			K = body.apply_status_effect(STATUS_EFFECT_KNOCKDOWN, amount, updating)
		return K
...
/mob/living/proc/IsImmobilized() //If we're immobilized
	return body.has_status_effect(STATUS_EFFECT_IMMOBILIZED)
...
/mob/living/proc/Immobilize(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		...
		if(I)
			...
			I = body.apply_status_effect(STATUS_EFFECT_IMMOBILIZED, amount, updating)
		return I
...
/mob/living/proc/IsParalyzed() //If we're immobilized
	return body.has_status_effect(STATUS_EFFECT_PARALYZED)
...
/mob/living/proc/Paralyze(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		...
		if(P)
			...
			P = body.apply_status_effect(STATUS_EFFECT_PARALYZED, amount, updating)
		return P
...
/mob/living/proc/IsUnconscious() //If we're unconscious
	return body.has_status_effect(STATUS_EFFECT_UNCONSCIOUS)
...
/mob/living/proc/Unconscious(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if(((status_flags & CANUNCONSCIOUS) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE))  || ignore_canstun)
		...
		if(U)
			...
			U = body.apply_status_effect(STATUS_EFFECT_UNCONSCIOUS, amount, updating)
		return U
...
/mob/living/proc/IsSleeping() //If we're asleep
	return body.has_status_effect(STATUS_EFFECT_SLEEPING)
...
/mob/living/proc/Sleeping(amount, updating = TRUE, ignore_canstun = FALSE) //Can't go below remaining duration
	...
	if((!HAS_TRAIT(src, TRAIT_SLEEPIMMUNE)) || ignore_canstun)
		...
		if(S)
			...
			S = body.apply_status_effect(STATUS_EFFECT_SLEEPING, amount, updating)
		return S
...
/mob/living/proc/IsFrozen()
	return body.has_status_effect(/datum/status_effect/freon)
...
/mob/living/proc/absorb_stun(amount, ignoring_flag_presence)
	if(amount < 0 || is_unconcious() || ignoring_flag_presence || !islist(stun_absorption))
		return FALSE
	if(!amount)
		amount = 0
...
/mob/living/proc/IsInStasis()
	. = body.has_status_effect(STATUS_EFFECT_STASIS)

/mob/living/proc/SetStasis(apply, updating = TRUE)
	. = apply ? body.apply_status_effect(STATUS_EFFECT_STASIS, null, updating) : body.remove_status_effect(STATUS_EFFECT_STASIS)
