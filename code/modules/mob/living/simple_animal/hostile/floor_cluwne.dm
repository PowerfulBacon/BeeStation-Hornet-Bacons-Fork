...
/mob/living/simple_animal/hostile/floor_cluwne/Life()
	...
	if(prob(5))//checks roughly every 20 ticks
		if(current_victim.is_dead() || current_victim.dna.check_mutation(CLUWNEMUT) || is_type_in_typecache(A, invalid_area_typecache) || !is_station_level(current_victim.z))
			if(!Found_You())
				Acquire_Victim()
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Acquire_Victim(specific)
	for(var/I in GLOB.player_list)//better than a potential recursive loop
		...
		if(specific)
			H = specific
			A = get_area(H.loc)
			if(H.is_alive() && H.has_dna() && !H.dna.check_mutation(CLUWNEMUT) && !is_type_in_typecache(A, invalid_area_typecache) && is_station_level(H.z))
				return target = current_victim

		A = get_area(H.loc)
		if(H && ishuman(H) && H.is_alive() && H != current_victim && H.has_dna() && !H.dna.check_mutation(CLUWNEMUT) && !is_type_in_typecache(A, invalid_area_typecache) && is_station_level(H.z))
			current_victim = H
			interest = 0
			stage = STAGE_HAUNT
			return target = current_victim
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Kill(mob/living/carbon/human/H)
	...
	if(do_after(src, 50, target = H))
		...
		if(prob(75))
			...
			H.body.apply_injury(BP_BRAIN, /datum/injury/organ_damage, 10)
			H.cure_blind()
