	...
	switch(stage)
		...
		if(STAGE_ATTACK)
			...
			if(!eating)
				...
				H.add_overall_injury(/datum/injury/brute/blunt/crush, 10, INJURY_SEVERITY_MINOR, 90)
				manifested = TRUE
				Manifest()
				if(!eating)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Kill(mob/living/carbon/human/H)
	...
	if(do_after(src, 50, target = H))
		...
		if(prob(75))
			...
			H.cluwneify()
			H.add_overall_injury(/datum/injury/brute/blunt/crush, 30, INJURY_SEVERITY_MINOR, 90)
