...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			for(var/mob/living/m in ohearers(1, src))
				m.add_bodypart_injury(pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_PRECISE_GROIN), /datum/injury/brute/blunt, 15, INJURY_SEVERITY_MINOR, 60)
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			throwSmoke(loc)
			for(var/mob/living/m in ohearers(1, src))
				m.add_bodypart_injury(pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_PRECISE_GROIN), /datum/injury/burn/freeze, 5, INJURY_SEVERITY_MINOR, 70)
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf > 36 && globalMalf < 50)
			...
			for(var/mob/living/m in hearers(4,src))
				...
				m.add_bodypart_injury(BODY_ZONE_CHEST, /datum/injury/brute/sharp, 50, INJURY_SEVERITY_MAJOR, 200)
