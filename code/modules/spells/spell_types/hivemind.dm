...
/obj/effect/proc_holder/spell/self/hive_drain/cast(mob/living/carbon/human/user)
	...
	if(!carbon_members.len)
		return
	if(!user.bruteloss && !user.fireloss && !user.cloneloss && !user.getOrganLoss(ORGAN_SLOT_BRAIN))
	...
	while(iterations < 7)
		...
		if(user.bruteloss > user.fireloss)
			user.heal_ordered_damage(5, list(CLONE, BRUTE, BURN))
		else
			user.heal_ordered_damage(5, list(CLONE, BURN, BRUTE))
		if(!user.bruteloss && !user.fireloss && !user.cloneloss) //If we don't have any of these, stop looping
