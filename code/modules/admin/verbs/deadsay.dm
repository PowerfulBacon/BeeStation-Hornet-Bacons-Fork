/client/proc/dsay(msg as text)
	...
	for (var/mob/M in GLOB.player_list)
		if(isnewplayer(M))
			continue
		if (M.is_dead() || (M.client && M.client.holder && (M.client.prefs.chat_toggles & CHAT_DEAD))) //admins can toggle deadchat on and off. This is a proc in admin.dm and is only give to Administrators and above
			to_chat(M, rendered)
