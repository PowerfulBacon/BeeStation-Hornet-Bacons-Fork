...
/obj/machinery/fugitive_capture/MouseDrop_T(mob/target, mob/user)
	...
	if(!isliving(fugitive_hunter))
		return
	if(fugitive_hunter.body.stat || (!(fugitive_hunter.mobility_flags & MOBILITY_STAND) || !(fugitive_hunter.mobility_flags & MOBILITY_UI)) || !Adjacent(fugitive_hunter) || !target.Adjacent(fugitive_hunter) || !ishuman(target))
		return
