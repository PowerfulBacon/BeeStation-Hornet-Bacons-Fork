...
/datum/surgery_step/dissection/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	SSresearch.science_tech.add_point_list(list(TECHWEB_POINT_TYPE_BIOLOGY = check_value(target)))
...
/datum/surgery_step/dissection/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	SSresearch.science_tech.add_point_list(list(TECHWEB_POINT_TYPE_BIOLOGY = (check_value(target) * 0.2)))
	var/obj/item/bodypart/L = target.get_bodypart(BODY_ZONE_CHEST)
	target.apply_damage(80, BRUTE, L)
	ADD_TRAIT(target, TRAIT_DISSECTED, "surgery")
	return TRUE
