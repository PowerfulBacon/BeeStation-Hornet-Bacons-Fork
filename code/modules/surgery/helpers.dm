	...
	if(!current_surgery)
		...
		if(P && user && user.Adjacent(M) && (I in user))
			...
			if(S.ignore_clothes || get_location_accessible(M, selected_zone))
				...
				I.balloon_alert(user, "Drapes placed over [parse_zone(selected_zone)]", "<span class='notice'>You drape over [parse_zone(selected_zone)]</span>", color = COLOR_BALLOON_INFOMATION)

				log_combat(user, M, "operated on", null, "(OPERATION TYPE: [procedure.name]) (TARGET AREA: [selected_zone])")
			else
				I.balloon_alert(user, "[parse_zone(selected_zone)] is covered", color = COLOR_BALLOON_WARNING)
...
/proc/attempt_cancel_surgery(datum/surgery/S, obj/item/I, mob/living/M, mob/user)
	...
	if(S.status == 1)
		...
		I.balloon_alert(user, "[I] removed", "<span class='notice'>You remove [I] from [parse_zone(selected_zone)]</span>", color = COLOR_BALLOON_INFOMATION)
		qdel(S)
		return
