...
/datum/dmscript/compiler_operation/divide
	//The regex to check for this operation
	//Just detect any brackets anywhere
	check_regex = new /regex(@"^(.*[^/])(?<!var)\/([^/].*)$", "m")
	//The priority to check this operation
	priority = PRECEDENCE_MULTIPLICATIVE

/datum/dmscript/compiler_operation/divide/process_operation(text_line, list/subparts)
	...
	return "[BYTECODE_DIVIDE] [left_group_identifier] [right_group_identifier]"
