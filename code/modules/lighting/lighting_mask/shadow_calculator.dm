 ...
 * This is the second most important part of the lighting system. It seems like it would be insanely expensive, but its just basic
 * matrix maths and other simple calculations which computers can handle really well.
 * The worst part is byond's view() and applying overlays.
 * In the future, a good way to optimise this would be to create a view() implementation that only considers a grid of points that are opaque
 * or not opaque. (View() returns everything in view, but we only want to see turfs, so the majority of the stuff returned by view() is redundant.)
 *
 */
