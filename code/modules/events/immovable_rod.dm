...
/obj/effect/immovablerod/proc/penetrate(mob/living/L)
	...
	if(ishuman(L))
		...
		H.add_overall_injury(/datum/injury/brute/sharp/stab, 160, INJURY_SEVERITY_CRITICAL, 80)
	...
	if(ishuman(user))
		...
		if(U.job in list("Research Director"))
			...
			if(wizard)
				...
				wizard.Stun(60)
				wizard.add_bodypart_injury(BODY_ZONE_CHEST, /datum/injury/brute/blunt/crush, 25, INJURY_SEVERITY_MINOR, 30)
				qdel(src)
			else
