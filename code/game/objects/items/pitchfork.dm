...
/obj/item/pitchfork/demonic/pickup(mob/living/user)
	...
	if(isliving(user) && user.mind && user.owns_soul() && !is_devil(user))
		...
		U.add_bodypart_injury(pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM), /datum/injury/burn, rand(force * 0.5, force), INJURY_SEVERITY_MINOR, 200)
...
/obj/item/pitchfork/demonic/attack(mob/target, mob/living/carbon/human/user)
	if(user.mind && user.owns_soul() && !is_devil(user))
		...
		user.add_bodypart_injury(pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM), /datum/injury/burn, rand(force * 0.5, force), INJURY_SEVERITY_MINOR, 200)
	..()
