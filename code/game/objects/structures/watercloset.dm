...
/obj/structure/toilet/attack_hand(mob/living/user)
	...
	if(swirlie)
		...
		swirlie.add_overall_injury(/datum/injury/brute/blunt, 5, INJURY_SEVERITY_MINOR, 10)
		...
		if(user.grab_state >= GRAB_AGGRESSIVE)
			...
			if(!swirlie)
				if(open)
					...
					GM.add_overall_injury(/datum/injury/brute/blunt, 5, INJURY_SEVERITY_MINOR, 10)
		else
...
/obj/structure/urinal/attack_hand(mob/user)
	...
	if(user.pulling && user.a_intent == INTENT_GRAB && isliving(user.pulling))
		...
		if(user.grab_state >= GRAB_AGGRESSIVE)
			...
			GM.add_overall_injury(/datum/injury/brute/blunt, 8, INJURY_SEVERITY_MINOR, 10)
		else
			to_chat(user, "<span class='warning'>You need a tighter grip!</span>")
