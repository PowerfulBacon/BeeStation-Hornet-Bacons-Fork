...
/proc/spawn_rivers(target_z, nodes = 4, turf_type = /turf/open/lava/smooth/lava_land_surface, whitelist_area = /area, min_x = RANDOM_LOWER_X, min_y = RANDOM_LOWER_Y, max_x = RANDOM_UPPER_X, max_y = RANDOM_UPPER_Y, new_baseturfs)
...
/turf/proc/Spread(probability = 30, prob_loss = 25, whitelisted_area)
	...
	for(var/turf/canidate as anything in RANGE_TURFS(1, src) - src)
		if(!canidate || (canidate.density && !ismineralturf(canidate)) || istype(canidate, /turf/open/indestructible))
			continue

		var/area/new_area = get_area(canidate)
		if((!istype(new_area, whitelisted_area) && whitelisted_area) || (canidate.flags_1 & NO_LAVA_GEN_1))
			continue
