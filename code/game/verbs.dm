...
/datum/proc/add_verb(new_verbs)
	...
	for(var/verb_in_list in new_verbs)
		var/datum/procpath_holder/V = new /datum/procpath_holder(verb_in_list)
		if (!V.category)
			continue

		// If we already have the category
		if(V.category in sorted_verbs)
			...
			BINARY_INSERT_TEXT(V, verbs, datum/procpath_holder, name)
			sorted_verbs["[V.category]"] = verbs
		// If we do not, create the category and sort it into the right place
		else
...
/client/proc/remove_verbs(old_verbs)
	...
	for(var/pp in old_verbs)
		var/datum/procpath_holder/P = new /datum/procpath_holder(pp)
		if(!P)
			continue
...
/client/proc/add_verbs(new_verbs)
	...
	for(var/pp in new_verbs)
		...
		added_verbs[P.category]["[P.name]"] = list(
			action = "verb",
			params = list("verb" = P.name, "subpaths" = P.subcategories),
			type = STAT_VERB,
		)
