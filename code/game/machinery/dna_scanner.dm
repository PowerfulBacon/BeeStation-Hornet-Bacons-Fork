...
/obj/machinery/dna_scannernew/container_resist(mob/living/user)
	...
	if(do_after(user,(breakout_time), target = src))
		if(!user || user.body.stat != CONSCIOUS || user.loc != src || state_open || !locked)
			return
		locked = FALSE
...
/obj/machinery/dna_scannernew/MouseDrop_T(mob/target, mob/user)
	...
	if(user.is_unconcious() || (isliving(user) && (!(L.mobility_flags & MOBILITY_STAND) || !(L.mobility_flags & MOBILITY_UI))) || !Adjacent(user) || !user.Adjacent(target) || !iscarbon(target) || !user.IsAdvancedToolUser())
		return
