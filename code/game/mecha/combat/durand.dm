...
/obj/durand_shield/proc/activate(datum/source, var/datum/action/innate/mecha/mech_defense_mode/button, list/signal_args)
	...
	if(!chassis.defense_mode && (!chassis.cell || chassis.cell.charge < 100)) //If it's off, and we have less than 100 units of power
		chassis.balloon_alert(chassis.occupant, "Insufficient power", color = COLOR_BALLOON_WARNING)
		return
	switching = TRUE
	...
	if(!signal_args[1])
		chassis.balloon_alert(chassis.occupant, "Shield [chassis.defense_mode?"enabled":"disabled"]", color = chassis.defense_mode?COLOR_BALLOON_ENABLED:COLOR_BALLOON_DISABLED)
